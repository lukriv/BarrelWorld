...
GameErrorCode InputComponent::CreateInputDef(wxXmlNode* pNode, InputDef& output)
{
	...
	if(child->GetName() == GAME_TAG_PARAM_INPUT_KEY) {
		...
		FWGLOG_ERROR_FORMAT(
		    wxT("Read key attributes failed: 0x%08x"), m_pInputSystem->GetLogger(), result, FWGLOG_ENDVAL);
		return result;
	    }

	    if(FWG_FAILED(result = output.m_inputMap.Insert(action, keyCode))) {
		FWGLOG_ERROR_FORMAT(wxT("Insert key action failed (action='%s' on line='%d'): 0x%08x"),
		                    m_pInputSystem->GetLogger(),
		                    action.GetData().AsInternal(),
		                    child->GetLineNumber(),
		                    result,
		                    FWGLOG_ENDVAL);
		return result;
