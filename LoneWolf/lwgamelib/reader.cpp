...
bool LoneWolfXmlReader::ParseScene(wxXmlNode* sceneNode, GlobalResourceManager& resMgr, SceneManager& sceneMgr)
{
	...
	while(child)
	{
		...
		} else if (child->GetName() == GENERAL_TAG_ITEMS_SELECTION_STR) {
			if(!ParseSceneSelectionItems(child, resMgr, *(spScene.get())))
			{
				return false;
			}
...
bool LoneWolfXmlReader::ParseSceneSelectionItems(wxXmlNode* sceneActionNode, GlobalResourceManager& resMgr, Scene& scene)
{
	wxString tempValue;
	wxString nameValue;
	long itemToSelect = 0;
	...
	if(!sceneActionNode->GetAttribute(GENERAL_ATTR_ITEMS_COUNT_STR, &tempValue))
	{
		ProcessError(sceneActionNode, wxT("items_cout attribute is missing"));
		return false;
	}
	
	tempValue.ToLong(&itemToSelect);
	scene.SetSelectionItemsCount(itemToSelect);
	...
	while(child)
	{
		if(child->GetName() == GENERAL_TAG_ITEM_STR)
		{
			...
			if(resMgr.GetItemAndDiscMgr().GetItemType(nameValue) != ITEM_UNKNOWN)
			{
				scene.AddItem(resMgr.GetItemAndDiscMgr().GetItemType(nameValue), true);
			} else {
				ProcessError(child);
				return false;
			}
...
bool LoneWolfXmlReader::ParseDefActions(wxXmlNode* defNode, GlobalResourceManager& resMgr)
{
	...
	while(child)
	{
		if((child->GetName() == GENERAL_TAG_ACTION_CREATE_STR)
			...
			||(child->GetName() == GENERAL_TAG_ACTION_CHAPTER_STR)
			||(child->GetName() == GENERAL_TAG_ACTION_PROMOTE_STR))
		{
			// read attributes
			descValue.Clear();
...
bool LoneWolfXmlReader::ReadItemAttr(wxXmlNode* itemNode, GlobalResourceManager& resMgr)
{
	...
	if(itemNode->HasAttribute(GENERAL_ATTR_ATTACK_ACT_STR))
	{
		tempValue = itemNode->GetAttribute(GENERAL_ATTR_ATTACK_ACT_STR);
		tempValue.ToLong(&intValue);
		item.m_properties.m_actualAttack = static_cast<wxInt32>(intValue);
	}
	...
	if(itemNode->HasAttribute(wxString(GENERAL_ATTR_WEAPON_TYPE_STR)))
	{
		// only weapons should have this attribute
		EWeaponClass weaponClass = WEAPON_CLASS_NONE;
		tempValue = itemNode->GetAttribute(GENERAL_ATTR_WEAPON_TYPE_STR);
		weaponClass = Convertor::ConvertWeaponClassToType(tempValue);
		if(weaponClass != WEAPON_CLASS_NONE)
		{
			item.m_weaponClass = weaponClass;
		} else {
			ProcessError(itemNode, wxT("Unknown weapon class"));
			return false;
		}
...
bool LoneWolfXmlReader::ReadDisciplineAttr(wxXmlNode* discNode, GlobalResourceManager& resMgr, DisciplineProperties& properties)
{
	...
	if(discNode->HasAttribute(wxString(GENERAL_ATTR_WEAPON_TYPE_STR)))
	{
		tempValue = discNode->GetAttribute(wxString(GENERAL_ATTR_WEAPON_TYPE_STR));
		properties.m_weaponClass.push_back(Convertor::ConvertWeaponClassToType(tempValue));
	}
